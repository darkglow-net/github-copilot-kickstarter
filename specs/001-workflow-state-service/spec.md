...
- HTTP/SSE via manual `StreamableHTTPServerTransport` with per-request session instantiation (MCP SDK v1.12.x)
...
**Implementation note**: Task ID allocation reads `progress_state_json` to compute the next available ID without mutating state. The caller creates the actual task entry separately via `update-state`. This is a pure read-only computation.
...
| `allocate-task-id` | true | false | true |
...
│   │   ├── context.ts         # Context memory helpers (store, get, briefing)
...
| Package | Purpose |
|---------|---------|
| `@modelcontextprotocol/sdk` | Consolidated MCP server + transport framework (v1.12.x stable). All features via subpath imports (`server/mcp.js`, `server/streamableHttp.js`) |
| `zod` | Schema validation (v4, via `zod/v4`) |
| `better-sqlite3` | SQLite driver |
| `kysely` | Type-safe query builder |
| `express` | HTTP framework for MCP transport |
...
|---|----------|----------|----------|
| 1 | Express vs native HTTP vs Fastify | Manual Express + `StreamableHTTPServerTransport` | Direct session lifecycle control via per-request transport instantiation. Manual approach provides visibility into MCP transport lifecycle (session tracking, cleanup) that the `createMcpExpressApp()` helper would hide. MCP SDK v1.12.x stable used instead of pre-release v2 modular packages. |
